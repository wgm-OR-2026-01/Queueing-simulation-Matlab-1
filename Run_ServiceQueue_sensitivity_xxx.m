Run samples of the ServiceQueue simulation department store Madeline Breiling
Collect statistics and plot histograms along the way.

Set up
We'll measure time in minutes
Arrival rate: 1 every 75 seconds = 0.8 every minute
lambda = 0.8;
Departure (service) rate: 1 per 6.5 minutes = 0.15 every minute
mu = 0.15;
...
%LogInterval = 1/60;
%log entry every 5 minutes
LogInterval = 5;


Numbers from theory for M/M/1 queue
Compute P(1+n) =  = probability of finding the system in state  in the long term. Note that this calculation assumes .
rho = lambda / mu;
P0 = 1 - rho;
nMax = 10;
P = zeros([1, nMax+1]);
...
This sets the vertical axis to go from  to .
ylim(ax, [0, 0.3]);
This sets the horizontal axis to go from  to .  The histogram will use bins  so this leaves some visual breathing room on the left.
xlim(ax, [-1, 21]);
...
1)
L: expected total number of customers in the system
L = = =5
: expected number of customers waiting (not being served)

W: expected total time in the system, waiting plus service time
...
%q.Served - c.ArrivalTime
%expectedWaitingTimeQueue = mean(cellfun(@(q) mean(q.Log.)))

customercount = length(TimeInQueue)
toolong = 0 
justfine = 0
for customer = 1:customercount
    if TimeInQueue(customer) > 5
        toolong = toolong + 1;
    else
        justfine = justfine + 1;
    end
end
fprintf("Num cust waited too long: %f\n", toolong)
fprintf("Num cust waited just fine: %f\n", justfine)
proptoolong = toolong/(toolong+justfine)