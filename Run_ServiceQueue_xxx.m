...
%[text] Arrival rate: every 75 seconds
lambda = 0.8;
%[text] Departure (service) rate: 6.5 minutes
mu = 1/6.5;
...
MaxTime = 240; %four hours in minutes
...
%%
%[text] ### Now to calculate the mean number of customers waiting in our simulation: 
...
%%
%[text] ### Now to calculate the mean waiting time in the system for our simulation: 
WqSamples = cellfun( ...
    @(q) cellfun(@(c) c.BeginServiceTime - c.ArrivalTime, q.Served'), ...
    QSamples, ...
    UniformOutput=false);

Wq_all = vertcat(WqSamples{:});

Wq_sim = mean(Wq_all);
Wq_requirement = mean(Wq_all>5);
fprintf("Simulated W_q: %f\n", Wq_sim);
if Wq_requirement <= 0.1
    fprintf('s=%d meets the requirement.\n',s);
end
%%
